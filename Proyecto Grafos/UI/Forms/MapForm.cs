...
namespace Proyecto_Grafos
{
    public partial class MapForm : Form, IMapView
    {
        ...
        // ── CAMBIO 1 ────────────────────────────────────────────────────────────
        // Se declaran recursos estáticos compartidos para evitar crear nuevas
        // instancias de Bitmap y Pen en cada llamada a DrawRoutes/DrawLabeledSegments.
        // Antes: se hacía "new Bitmap(1, 1)" y "new Pen(...)" dentro de cada loop,
        // generando objetos no liberados en el heap con cada clic del usuario.
        private static readonly Bitmap _emptyBitmap = new Bitmap(1, 1);
        private static readonly Pen _redRoutePen = new Pen(Color.Red, 3);
        private static readonly Pen _blueRoutePen = new Pen(Color.Blue, 3);
        // ────────────────────────────────────────────────────────────────────────
        ...
        public void DrawRoutes(List<List<PointLatLng>> routes)
        {
            ...
            foreach (var routePoints in routes)
            {
                ...
                // ── CAMBIO 2 ──────────────────────────────────────────────────────
                // Antes: { Stroke = new Pen(Color.Red, 3) }
                // Ahora: se reutiliza el Pen estático _redRoutePen declarado arriba.
                // Esto evita crear un objeto Pen nuevo (y no liberado) por cada ruta.
                var route = new GMapRoute(routePoints, "route") { Stroke = _redRoutePen };
                // ──────────────────────────────────────────────────────────────────
                ...
                // ── CAMBIO 3 ──────────────────────────────────────────────────────
                // Antes: new GMarkerGoogle(midPoint, new Bitmap(1, 1))
                // Ahora: se reutiliza el Bitmap estático _emptyBitmap.
                // Con 100 personas esto evita crear ~100 Bitmaps por cada clic.
                var distanceMarker = new GMarkerGoogle(midPoint, _emptyBitmap)
                // ──────────────────────────────────────────────────────────────────
                {
        ...
        public void DrawLabeledSegments(List<RouteSegment> segments)
        {
            ...
            foreach (var segment in segments)
            {
                ...
                // ── CAMBIO 4 ──────────────────────────────────────────────────────
                // Antes: { Stroke = new Pen(Color.Blue, 3) }
                // Ahora: se reutiliza el Pen estático _blueRoutePen declarado arriba.
                var route = new GMapRoute(routePoints, "segment") { Stroke = _blueRoutePen };
                // ──────────────────────────────────────────────────────────────────
                _routesOverlay.Routes.Add(route);
                ...
                // Antes: new GMarkerGoogle(midPoint, new Bitmap(1, 1))
                // Ahora: se reutiliza el Bitmap estático _emptyBitmap.
                var labelMarker = new GMarkerGoogle(midPoint, _emptyBitmap)
                // ──────────────────────────────────────────────────────────────────
                {
        ...
        // ── CAMBIO 6 ────────────────────────────────────────────────────────────
        // Métodos nuevos: BeginUpdate / EndUpdate.
        // Permiten que el MapPresenter suspenda el repintado del control durante
        // la carga masiva de marcadores, evitando repintados intermedios por cada
        // marcador agregado. Se llaman desde LoadDataAndRefreshView en el Presenter.
        public void BeginUpdate() => gMapControl1.SuspendLayout();
        public void EndUpdate() => gMapControl1.ResumeLayout(false);
        // ────────────────────────────────────────────────────────────────────────
        ...
        // ── CAMBIO 7 ────────────────────────────────────────────────────────────
        // Antes: OnFormClosed solo llamaba _presenter.Dispose().
        // Ahora: también limpia explícitamente todos los overlays y vacía el caché
        // de tiles en memoria de GMap (KiberTileCache), que según el análisis de
        // rendimiento ocupaba el 36.9% del heap al cerrar el formulario.
        protected override void OnFormClosed(FormClosedEventArgs e)
        {
            base.OnFormClosed(e);
            _presenter?.Dispose();

            // Limpia los overlays para liberar referencias a markers y rutas
            _markersOverlay.Markers.Clear();
            _routesOverlay.Routes.Clear();
            _routesOverlay.Markers.Clear();
            _tempOverlay.Markers.Clear();

            // Vacía el caché de tiles en memoria para reducir el remanente en heap
